...
- [x] `basic_utils.py` HF_ENDPOINT bypass: `myTokenizer.__init__` ve `create_model_and_diffusion`'da PubMedBERT yüklemesi sırasında `HF_ENDPOINT` geçici kaldırılıyor (hf-mirror PubMedBERT servis etmiyor, `OSError` veriyordu)
- [x] `train.py` + `sample_vqa_GPU.py` başındaki `HF_ENDPOINT=hf-mirror` set'i kaldırıldı — kök neden bu satırdı, PubMedBERT from_pretrained tüm process boyunca hf-mirror'a gidiyordu
